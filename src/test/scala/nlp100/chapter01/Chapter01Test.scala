...
class Chapter01Test extends FunSuite with Matchers {
  ...
  test("08. 暗号文") {
    ...
    def cipher(text: String): String = {
      // MEMO: map は出来るだけ元の型を維持しようとするので、最後に toString は必要無い
      // MEMO:
      text.map { c => if (c.isLower) { (219 - c.toInt).toChar } else { c } }
    }

    cipher(cipher("I love Scala. Do you like Scala?")) shouldBe "I love Scala. Do you like Scala?"
